...
public:
    ...
    WeightedGraphVertex<V, E> *&getAnotherEnd(WeightedGraphVertex<V, E> *v)//這是個參照 請跟他打招呼 說hi 參照
    {
        if (v != end[0])
            return end[0];
        return end[1];
...
public:
    ...
    void DFS(WeightedGraphVertex<V, E> *v)
    {
        LinkList<WeightedGraphVertex<V, E> *> *l = new LinkList<WeightedGraphVertex<V, E> *>();
        l->addFromHead(v);
        ListNode<WeightedGraphVertex<V, E> *> *cur = l->gethead();
        LinkList<WeightedGraphVertex<V, E> *> *tmp = new LinkList<WeightedGraphVertex<V, E> *>();
        while(cur)
        {
            if(!tmp->exist(cur->getData()))
            {
                tmp->addFromHead(cur->getData());
            }
            else
            {
                l->removeFromHead();
            ...
            cout << cur << " ";
            l->removeFromHead();
            ListNode<WeightedGraphEdge<V, E> *> *e = (*(cur->getData()))[0]; // (*(cur->getData()))[0]存的是listnode
            while(e)
            {
                l->addFromHead((e->getData())->getAnotherEnd(cur->getData()));
                e = e->getNext();
            }
           // l->print();
