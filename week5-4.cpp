...
public:
    ...
    void push(T data){list->addToTail(data);}
    T pop(){return list->removeFormTail()->getData();}
    T top(){return list->getTail()->getData();}
    bool isEmpty()
    {
...
class BigDecimal
{
  ...
  void set_symbol(bool set){this->symbol = set;}
...
BigDecimal *stob(string data)
{
    ...
    for(int i = 0 ; i < size ; ++i)
    {
        ...
        else if(isdecimal) {tmp += data[i]; temp->decimal->addToTail(tmp);}
        else {tmp += data[i]; temp->integer->addToTail(tmp);}
    }
...
int main() 
{
    ...
    while(cin>>input)
    {
        ...
        if(err_exp(input))
            continue;
        cout<<evalue(change_impression(input));
    }
    return 0;
}
